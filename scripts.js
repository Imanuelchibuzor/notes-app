...
addBox.addEventListener('click', () => {
  titleTag.focus(); // This adds the cursor in the Title input area
...
function showNotes() {
  document.querySelectorAll('.note').forEach(note => note.remove()); // removing any duplicate note
...
function updateNote(noteId, title, desc) {
  isUpdate = true;
  updateId = noteId; // storing id of the note for updating purpose
  addBox.click();
...
addBtn.addEventListener('click', e => {
  ...
  if(noteTitle || noteDesc) {
    ...
    localStorage.setItem('notes', JSON.stringify(notes));
    closeIcon.click(); // calling the addEventListener for closeIcon as a function
    showNotes(); // calling the showNotes function to display updated notes
  }
});